...
export default function CoursesIndexPage () {
  ...
  const filtered = useMemo(() => {
    if (!activeCategory) return mockCourses
    const list = mockCourses.filter(c => c.category === activeCategory || c.title.includes(activeCategory))
        ...
        <div className='px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl'>
          <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
            <div>
              ...
              {activeCategory && (
                <p className='mt-1 text-sm text-gray-600 text-right'>تُعرض الآن فئة: {activeCategory}</p>
              )}
            </div>

            <div className='flex flex-wrap gap-2 justify-end'>
              <Link href='/courses' className={`px-3 py-1.5 rounded-full text-sm border ${!activeCategory ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-700 hover:border-gray-300'}`}>
                الكل
              </Link>
              {categories.map(cat => (
                <Link
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm border ${activeCategory === cat ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-700 hover:border-gray-300'}`}
                >
                  {cat}
                </Link>
              ))}
