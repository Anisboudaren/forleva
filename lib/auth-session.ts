...
const COOKIE_NAME = 'forleva_admin_session'
...
export type AdminSession = { userId: string; role: 'SUPER_ADMIN' | 'ADMIN' | 'TEACHER'; email: string | null }

export function setAdminSessionCookie(session: AdminSession): string {
...
export function parseAdminSession(cookieValue: string): AdminSession | null {
  const payload = verify(cookieValue)
  if (!payload) return null
  try {
    const decoded = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'))
    if (decoded.userId && (decoded.role === 'SUPER_ADMIN' || decoded.role === 'ADMIN' || decoded.role === 'TEACHER')) {
      return {
